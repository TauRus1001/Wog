...
function arm_view(t, e, r, i, o) {
    ...
    "0" != e)
        for (var n = t.split(";"), a = 0, l = 0, s = 0; s < n.length; s++) {
            ...
            for (var u = 0; u < c; u++) {
                ...
                if (l++,
                w = "" + att[p[16]],
                b = p[16] > 0 ? "<img src='/img/att.jpg' alt='" + w + "' title='" + w + "' width='16'>" : "",
                null != m)
                    var _ = "*" + m[u];
                else
                    _ = "";
                ...
                d.write("<tr " + g + " onMouseOver=\"this.style.backgroundColor='#777779'\" onMouseOut=\"this.style.backgroundColor=''\" onclick=\"parent.setRadioValue('adds'," + p[0] + ");parent.setCheckBoxValue(" + a + ')" style="cursor:hand"><td>' + l + '</td><td><input type="radio" name="adds" value="' + p[0] + '" data-name="' + p[5] + '"></td><td>' + p[6] + "</td><td>" + p[7] + "</td><td>" + p[1] + "</td><td>" + p[2] + "</td><td>" + p[3] + "</td><td><a href=\"javascript:parent.data_send2('arm','check_arm','" + r + "'," + p[0] + "," + l + ')">' + p[5] + "</a>" + _ + b + "</td><td>" + p[4] + "</td><td>" + p[15] + "</td><td>" + p[17] + "</td><td>" + p[18] + '</td><td><input type="checkbox" name="items[]" id="listKey-' + a + '" value="' + p[0] + '"></td></tr>'),
                a++
            }
        }
    else
        d.write("<tr><td colspan=13>裝備欄內沒有任何裝備</td></tr>");
    n = o.split(",");
    ...
    d.write('<tr><td colspan="13" ><a href="javascript:parent.wog_view.document.body.getElementsByClassName(\'scrollable-table\')[0].scrollTop=0" id="bottom" style="margin-right:10px;">回到頂部</a>欲轉移需輸入對方遊戲的帳號 <input type="text" name="pay_id" size="16"> 安全密碼 <input type="password" name="pw" size="16"></td></tr>'),
    d.write(temp_table2),
    ...
    addItemCss(),
    "d_item_id" != r && (parent.wog_view.document.head.getElementsByTagName("style")[1].innerHTML = ".scrollable-table{height:72%;overflow-y:auto;}.scrollable-table>form>#bagList{width:100%}th{background:#083118;position:sticky;top:0;font-size: 11pt;}table#bagList>tbody>tr:nth-last-of-type(-n+2){position: sticky;bottom: 27;background:#000000;}table#bagList>tbody>tr:last-child{position:sticky;bottom:0;background:#000000;}");
